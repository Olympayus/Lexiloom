...
> ⚠️ **测试项说明**：设置面板中的「在线词典查询」（牛津 / 朗文 / 柯林斯 / 韦氏）目前**仅为测试项**，尚未内置在线词典搜索功能；实际搜索完全基于本地 ECDICT / WordNet，该开关暂不生效。
...
| 系统 | 安装包 |
|------|--------|
| **Windows** | `Lexiloom_0.3.2_x64-setup.exe`（当前发布提供；MSI 暂未随包） |
| **macOS** | `Lexiloom_0.3.2_x64.dmg`（在 macOS 上构建） |
| **Linux** | `Lexiloom_0.3.2_x86_64.AppImage` / `.deb`（在 Linux 上构建） |
