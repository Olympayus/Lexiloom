...
2. `CLAUDE.md` 顶部 `Status:` 行、`README.md` 徽章 `version-<v>` 与安装表文件名：同步版本号与当前进度（README 已无 `Status` 段，以徽章 + 安装包名为准）
...
## 打包与发布
- 构建 Windows 安装包：`npm run tauri build`；网络受限时可只打 NSIS：`npm run tauri build -- --bundles nsis`（MSI 需联网下载 WiX，可能失败）
- 产物：`src-tauri/target/release/bundle/nsis/Lexiloom_<version>_x64-setup.exe`
- 发布 GitHub Release（push 与 gh 操作由用户亲自执行）：
  ```bash
  git tag v<version> && git push origin v<version>
  gh release create v<version> "src-tauri/target/release/bundle/nsis/Lexiloom_<version>_x64-setup.exe" \
    --title "Lexiloom v<version>" --notes "..."
  ```
- 常用脚本：`npm run dev`（Vite）、`npm run build`（tsc + vite）、`npm test`（vitest）、`npm run lint`（oxlint）、`npm run build:dictionaries`（构建本地词典库）
