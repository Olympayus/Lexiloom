...
> **Status: v0.4.3** — 开发中。v0.4.3 更新清单：搜索设置新增「词典」开关（ECDICT / WordNet，仅影响词典详情数据、语义网络不受影响）与「词源相关词」返回开关（原派生词入开关 + 提示小窗口）；搜索建议过滤无详情的孤儿词（如 applejacks）；右键全局禁止浏览器默认菜单 + 输入框自定义复制/粘贴菜单；合并添加后侧边栏音标/词性即时刷新；编辑页卡片 hover 按钮横向移出即消失；语义网络多词胶囊内单词独立可点、单击跳转对应词；合并添加按钮贴底；编者模式单行容器（无子项非词性）添加/删除按钮内联行尾。
