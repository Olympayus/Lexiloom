...
> **Status: v0.4.1** — 开发中。v0.4.1 更新清单：更新弹窗独立设计（应用级模态 + 进度可视化 + 超时/取消 + 代理提示 + 通用 ConfirmDialog 统一替换原生 confirm）、编辑视图标签页系统（词性+补充主标签页，短语/词形变化/派生词独立标签页，内容平铺 + 直接添加项 + 右键菜单删除）、标题区音标内联编辑（多音标）、删除单词改标题旁垃圾桶按钮。
